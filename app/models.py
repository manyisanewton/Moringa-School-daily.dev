...
class AuditLog(db.Model):
    ...
    def __repr__(self):return f"<AuditLog id={self.id} action={self.action} user_id={self.user_id}>"
class User(db.Model):
    __tablename__="users"
    id=db.Column(db.Integer,primary_key=True)
...
class UserProfile(db.Model):
    ...
    def __repr__(self):return f"<UserProfile id={self.id} user_id={self.user_id}>"
class Role(db.Model):
    __tablename__="roles"
    id=db.Column(db.Integer,primary_key=True)
...
class UserRole(db.Model):
    ...
    def __repr__(self):return f"<UserRole user_id={self.user_id} role_id={self.role_id}>"
class RolePermission(db.Model):
    ...
    def __repr__(self):return f"<RolePermission role_id={self.role_id} permission_id={self.permission_id}>"
class Category(db.Model):
    ...
    created_at=db.Column(db.DateTime,server_default=func.now(),nullable=False)
    contents=db.relationship("Content",back_populates="category",cascade="all, delete-orphan")
...
class Tag(db.Model):
    ...
    id=db.Column(db.Integer,primary_key=True)
    name=db.Column(db.String(64),unique=True,nullable=False)
    contents=db.relationship("ContentTag",back_populates="tag",cascade="all, delete-orphan")
...
class Content(db.Model):
    ...
    status=db.Column(db.Enum(ContentStatusEnum),default=ContentStatusEnum.Draft,nullable=False)
    ...
    history=db.relationship("ContentHistory",back_populates="content",cascade="all, delete-orphan")
    tags=db.relationship("ContentTag",back_populates="content",cascade="all, delete-orphan")
    comments=db.relationship("Comment",back_populates="content",cascade="all, delete-orphan")
    subscriptions=db.relationship("Subscription",back_populates="content",cascade="all, delete-orphan")
    wishlist_items=db.relationship("Wishlist",back_populates="content",cascade="all, delete-orphan")
    reactions=db.relationship("Reaction",back_populates="content",cascade="all, delete-orphan")
    flags=db.relationship("Flag",back_populates="content",cascade="all, delete-orphan")
    notifications=db.relationship("Notification",back_populates="content",cascade="all, delete-orphan")
    def __repr__(self):return f"<Content id={self.id} title={self.title}>"
class ContentTag(db.Model):
    ...
    __table_args__=(db.UniqueConstraint("content_id","tag_id",name="uq_content_tag"),)
    content=db.relationship("Content",back_populates="tags")
    tag=db.relationship("Tag",back_populates="contents")
    def __repr__(self):return f"<ContentTag content_id={self.content_id} tag_id={self.tag_id}>"
class ContentHistory(db.Model):
    ...
    def __repr__(self):return f"<ContentHistory id={self.id} content_id={self.content_id}>"
class Comment(db.Model):
    ...
    children=db.relationship("Comment",backref=db.backref("parent",remote_side=[id]),cascade="all, delete-orphan")
    def __repr__(self):return f"<Comment id={self.id} content_id={self.content_id}>"
class Subscription(db.Model):
    ...
    content_id=db.Column(db.Integer,db.ForeignKey("contents.id",ondelete="CASCADE"),nullable=True)
    content=db.relationship("Content",back_populates="subscriptions")
    __table_args__=(Index("ix_subscriptions_user_id","user_id"),Index("ix_subscriptions_category_id","category_id"),)
    def __repr__(self):return f"<Subscription user_id={self.user_id} category_id={self.category_id}>"
class Wishlist(db.Model):
    ...
    def __repr__(self):return f"<Wishlist id={self.id} user_id={self.user_id}>"
class Reaction(db.Model):
    ...
    def __repr__(self):return f"<Reaction id={self.id} type={self.type.value}>"
class Flag(db.Model):
    __tablename__="flags"
    id=db.Column(db.Integer,primary_key=True)
    user_id=db.Column(db.Integer,db.ForeignKey("users.id",ondelete="CASCADE"),nullable=False)
    ...
    def __repr__(self):return f"<Flag id={self.id} reason={self.reason}>"
class Notification(db.Model):
    ...
    def __repr__(self):return f"<Notification id={self.id} message={self.message}>"
class RefreshToken(db.Model):
    ...
    def __repr__(self):return f"<RefreshToken jti={self.token} revoked={self.revoked}>"
class PasswordResetToken(db.Model):
    ...
    def __repr__(self):return f"<PasswordResetToken id={self.id} user_id={self.user_id}>"
class EmailVerificationToken(db.Model):
    ...
    def __repr__(self):return f"<EmailVerificationToken id={self.id} user_id={self.user_id}>"
