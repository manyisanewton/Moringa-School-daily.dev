...
class CreateUserSchema(Schema):
    ...
    password = fields.Str(
        ...
        validate=[
            ...
            validate.Regexp(r".*\d.*", error="Password must contain at least one digit"),
        ],
    )
    roles = fields.List(
        fields.Str(),
...
def create_user():
    ...
    user = User(email=data["email"], name=chosen_name, password_hash=pw_hash)
    db.session.add(user)
    try:
        db.session.flush()
...
def promote_user(user_id: int, role_name: str):
    ...
    try:
        ...
        db.session.rollback()
        logger.exception("Failed to promote user %s to %s", user_id, role_name)
...
class FlagContentSchema(Schema):
    reason = fields.Str(required=True, validate=validate.Length(max=256))
...
def flag_content(content_id: int):
    ...
    db.session.commit()
    return jsonify(
        {"message": f"Content {content.id} flagged.", "reason": data["reason"]}
    ), 200
