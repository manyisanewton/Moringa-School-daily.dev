...
def create_app(config_class=DevConfig):
    ...
    oauth.register(name="github", client_id=app.config["GITHUB_CLIENT_ID"], client_secret=app.config["GITHUB_CLIENT_SECRET"], authorize_url="https://github.com/login/oauth/authorize", access_token_url="https://github.com/login/oauth/access_token", api_base_url="https://api.github.com/", client_kwargs={"scope":"user:email"})
    from app.models import RefreshToken
    @jwt.token_in_blocklist_loader
    def _check_revoked(jwt_header, jwt_payload):
        if jwt_payload.get("type") != "refresh": return False
    ...
    for bp in (auth_bp, user_bp, comments_bp, reactions_bp, content_bp, admin_bp, categories_bp, subscriptions_bp, notifications_bp, audit_bp, email_verification_bp, password_reset_bp, wishlists_bp):
