...
def seed_roles(target, connection, **kw):
    ...
    if count == 0:
        for role in UserRoleEnum:
            connection.execute(target.insert().values(name=role.value, description=role.value))
...
class Reaction(db.Model):
    ...
    def __repr__(self):
        return f"<Reaction id={self.id} user_id={self.user_id} content_id={self.content_id}>"
...
class Post(db.Model):
    __tablename__ = "posts"
    id = db.Column(db.Integer, primary_key=True)
    category_id = db.Column(db.Integer, db.ForeignKey("categories.id", ondelete="SET NULL"), nullable=False)
    ...
    post_comments = db.relationship("Comment", back_populates="post", cascade="all, delete-orphan")
    likes_rel = db.relationship("Like", back_populates="post", cascade="all, delete-orphan")
...
class Comment(db.Model):
    ...
    __table_args__ = (
        db.CheckConstraint(
            "(content_id IS NOT NULL AND post_id IS NULL) OR (content_id IS NULL AND post_id IS NOT NULL)",
