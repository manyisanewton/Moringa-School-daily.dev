...
comments_bp = Blueprint("comments", __name__, url_prefix="/categories/<int:post_id>/comments")
...
def create_comment(post_id: int):
    ...
    comment = Comment(post_id=post_id, user_id=user_id, body=data["body"], parent_id=parent_id)
    ...
    try:
        ...
        db.session.rollback()
        logger.exception("Failed to create comment for post %s by user %s: %s", post_id, user_id, str(e))
...
def list_comments(post_id: int):
    ...
    logger.info(f"Found {len(comments)} comments for post_id: {post_id}")
...
def edit_comment(post_id: int, comment_id: int):
    ...
    try:
        ...
        db.session.rollback()
        logger.exception("Failed to edit comment %s by user %s: %s", comment_id, user_id, str(e))
...
def delete_comment(post_id: int, comment_id: int):
    ...
    try:
        ...
        db.session.rollback()
        logger.exception("Failed to delete comment %s by user %s: %s", comment_id, user_id, str(e))
