...
comments_bp = Blueprint(
    "comments",
    __name__,
    url_prefix="/content/<int:content_id>/comments"
)
...
class CommentService:
    @staticmethod
    def get_content_or_404(content_id: int) -> Content:
        content = db.session.get(Content, content_id)
        if not content:
            abort(404, description="Content not found")
        return content
    @staticmethod
    def get_comment_or_404(content_id: int, comment_id: int) -> Comment:
        comment = Comment.query.filter_by(
            id=comment_id, content_id=content_id
        ).first()
        if not comment:
...
@jwt_required()
def create_comment(content_id: int):
    user_id = int(get_jwt_identity())
    CommentService.get_content_or_404(content_id)
    data = CommentSchema().load(request.get_json() or {})
    parent_id = data.get("parent_id")
    if parent_id:
        exists = Comment.query.filter_by(
            id=parent_id, content_id=content_id
        ).first()
        if not exists:
    ...
    try:
        ...
        db.session.rollback()
        logger.exception("Failed to create comment %s by %s", content_id, user_id)
...
@jwt_required()
def edit_comment(content_id: int, comment_id: int):
    user_id = int(get_jwt_identity())
    comment = CommentService.get_comment_or_404(content_id, comment_id)
...
@jwt_required()
def delete_comment(content_id: int, comment_id: int):
    user_id = int(get_jwt_identity())
    comment = CommentService.get_comment_or_404(content_id, comment_id)
