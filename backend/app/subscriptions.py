...
def list_subscriptions():
    ...
    subs = Subscription.query.filter_by(user_id=user_id).join(Category).all()
    result = [
        {
...
def subscribe(category_id):
    ...
    try:
        ...
        db.session.rollback()
        logger.exception(
            "Failed to subscribe user %s to category %s", user_id, category_id
        )
...
def unsubscribe(category_id):
    ...
    try:
        ...
        db.session.rollback()
        logger.exception(
            "Failed to unsubscribe user %s from category %s", user_id, category_id
        )
