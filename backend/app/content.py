...
def list_content():
    ...
    per_page = request.args.get("per_page", type=int, default=current_app.config.get("CONTENT_PER_PAGE", 10))
...
def get_techwriter_stats():
    ...
    likes = db.session.query(func.count(Reaction.id)).filter(
        Reaction.content_id.in_(db.session.query(Content.id).filter_by(author_id=user_id)),
...
def get_flagged_content():
    ...
    user = db.session.get(User, user_id) or abort(404, description="User not found")
    _require_writer_or_admin(user)
    flagged_content = Content.query.filter(Content.author_id == user_id, Content.status == ContentStatusEnum.Flagged).all()
    items = [
        {
            "id": c.id,
...
def update_content(content_id: int):
    ...
    if "status" in data:
        ...
        if data["status"] == ContentStatusEnum.Published:
    ...
    return jsonify(id=content.id, status=content.status.value), 200
...
def add_comment(content_id: int):
    user_id = int(get_jwt_identity())
    user = db.session.get(User, user_id) or abort(401, description="Invalid user")
    content = db.session.get(Content, content_id) or abort(404, description="Content not found")
    data = CommentSchema().load(request.get_json() or {})
    parent_id = data.get("parent_id")
    if parent_id:
        parent_comment = Comment.query.filter_by(id=parent_id, content_id=content_id).first()
        if not parent_comment:
            abort(400, description="Invalid parent comment")
    comment = Comment(content_id=content_id, user_id=user_id, body=data["body"], parent_id=parent_id)
    db.session.add(comment)
    try:
        db.session.commit()
    except SQLAlchemyError as e:
        db.session.rollback()
        logger.exception("Failed to create comment for content %s by user %s: %s", content_id, user_id, str(e))
...
def list_comments(content_id: int):
    logger.info(f"Fetching comments for content_id: {content_id}")
    ...
    logger.info(f"Found {len(comments)} comments for content_id: {content_id}")
