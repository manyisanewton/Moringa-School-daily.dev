...
export const getComments = async (contentId) => {
  try {
    ...
    console.error('Error fetching content comments:', {
      contentId,
...
export const getPostComments = async (postId) => {
  try {
    const response = await api.get(`/categories/${postId}/comments`);
    return response.data;
  } catch (error) {
...
export const getArticleComments = async (articleId) => {
  try {
    ...
    console.error('Error fetching article comments:', error.response?.data || error.message);
    throw error;
  }
};
